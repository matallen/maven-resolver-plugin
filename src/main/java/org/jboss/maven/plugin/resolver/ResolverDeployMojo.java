 ...
 * 
 */
public class ResolverDeployMojo extends ResolverMojo {
		...
		for (ModuleReference module : modules.values())
			MojoExecutor.executeMojo(MojoExecutor.plugin(MojoExecutor
				...
				MojoExecutor.configuration(
						MojoExecutor.element(MojoExecutor.name("file"),module.getSource().getAbsolutePath()), 
						MojoExecutor.element(MojoExecutor.name("repositoryId"), repositoryId),
						MojoExecutor.element(MojoExecutor.name("url"),repositoryUrl), 
						MojoExecutor.element(MojoExecutor.name("pomFile"), module.getDestinationPom().getAbsolutePath())), MojoExecutor.executionEnvironment(project, session, pluginManager));
	}
